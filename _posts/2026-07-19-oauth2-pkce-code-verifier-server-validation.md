...
검증을 통과한 시도는 동시 callback도 한 번만 통과하도록 소비 상태를 선점한다. 그 뒤 verifier를 복호화해 code·같은 redirect URI·verifier로 교환하고, token 응답은 transaction 안에서 암호화해 저장한다. 저장이 실패하면 연결 완료를 보여주지 않고 verifier를 삭제한다.

사용자 취소나 OAuth 서버 거절 callback에는 `code` 대신 `error`와 state가 올 수 있다. 이 경로도 건너뛰지 않는다. state가 있으면 세션·provider·만료·미사용 검증 뒤 시도를 소비하고 verifier 암호문을 삭제한다. token endpoint는 **한 번도 호출하지 않으며**, 사용자에게 provider 원문 대신 안전한 실패 안내만 보낸다. state가 없거나 검증에 실패해도 교환하지 않고 실패 이벤트만 남긴다. 오류 응답도 성공 응답과 같은 state 검증 경계에 둔다.
...
RFC 9700은 최초 요청의 challenge와 verifier 교환을 연결해 PKCE downgrade를 막도록 한다. 이 시나리오도 처음 저장한 시도와 지금 callback이 같은 한 건인지 확인한다.
...
| error callback | 유효한 state와 `error=access_denied`만 있는 callback을 보낸다. | 같은 state 검증·소비·verifier 삭제 뒤 안전한 실패 화면으로 끝나며 token endpoint 호출 횟수는 0이다. |
